    ...
    <?php else: ?>
        <ul class="divide-y divide-gray-200 dark:divide-gray-700 contacts-list">
            <?php foreach ($contacts as $contact): ?>
                <li>
                    <a href="/whatsup/chat_private?user=<?php echo urlencode($contact['id']); ?>" class="flex items-center gap-3 px-2 py-2 hover:bg-amber-50 dark:hover:bg-gray-800 rounded transition">
                        <img src="/whatsup/<?php echo $contact['avatar'] ?: 'storage/avatars/avatar_default.png'; ?>" alt="<?php echo htmlspecialchars($contact['displayName']); ?>" class="w-8 h-8 rounded-full object-cover">
